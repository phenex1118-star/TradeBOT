...
st.markdown("""
    <style>
    /* 修正頂部切邊問題：給予 3.5rem 讓上方群組按鈕有呼吸空間 */
    .block-container {
...
DEFAULT_GROUP_DATA = {
    ...
    "sell_ma": {"val": 10, "active": False},           # 新增：賣出均線
    "sell_trailing": {"val": 8.0, "active": False},    # 新增：移動停損%
    "watch_list": []
}
...
if not has_tg_credentials and not st.session_state.skip_login:
    ...
    with col2:
        ...
        with c_btn1:
            if st.button("🔓 登入", type="primary", use_container_width=True) and temp_token and temp_chat_id and not input_error:
...
if st.button(f"🔄 立即刷新 {display_name} 數據與通報", type="secondary", use_container_width=True):
    with st.spinner("獲取行情數據與驗證指標中..."):
        ...
        for stock_id in grp_data["watch_list"]:
            try:
                ...
                if "Too Many Requests" in str(e) or "429" in str(e): st.error(f"⚠️ {stock_id} 請求過於頻繁觸發限流，請等10秒重試。")
                else: st.error(f"標的 {stock_id} 數據異常: {e}")
                
        # 渲染 HTML 報表 (強制無縮排防 Bug)
        if summary_data:
            html_table = """<table style="width:100%; text-align:left; color:#FFFFFE; border-collapse: collapse; font-family: sans-serif; font-size:14px;">
...
<th style="padding: 12px 8px; min-width:110px;">近 5 日量能K棒</th>
...
with c_add1: new_stock = st.text_input("輸入代碼", placeholder="請輸入純數字", key="add_stock").strip()
with c_add2:
    st.write("") 
    if st.button("➕ 新增標的", use_container_width=True) and new_stock:
        if re.search(r'[\u4e00-\u9fff]', new_stock): st.error("⚠️ 請輸入純股票代碼 (例:2330)，不支援中文。")
        else:
            fmt_stock = f"{new_stock}.TW"
