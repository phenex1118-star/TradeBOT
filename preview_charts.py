...
DEFAULT_GROUP_DATA = {
    ...
    "spark_days": {"val": 5, "active": True},          # 新增：迷你 K 棒顯示天數
...
# 雲端存檔功能：利用 TG Chat ID 建立個人專屬設定檔
...
if not has_tg_credentials and not st.session_state.skip_login:
    ...
    with col2:
        st.title("📈 策略工作站系統登入")
        st.write("請輸入您的 Telegram 通訊憑證。系統將自動為您綁定個人專屬設定檔。")
        ...
        with c_btn1:
            if st.button("🔓 登入", type="primary", use_container_width=True) and temp_token and temp_chat_id and not input_error:
                # 登入時檢查是否有專屬存檔，有的話直接無縫恢復所有資料
                filename = f"userdata_{temp_chat_id}.json"
...
if st.button(f"🔄 立即刷新 {display_name} 數據與通報", type="secondary", use_container_width=True):
    with st.spinner("🚀 極速批次下載資料中，100% 免疫 API 限流..."):
        summary_data = []
